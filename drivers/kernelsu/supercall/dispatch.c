...
static bool ksu_selinux_initialized __read_mostly = false;
#endif
...
static int do_grant_root(void __user *arg)
{
...
#ifndef KSU_KPROBES_HOOK
	if (!ksu_selinux_initialized) {
		ksu_selinux_initialized = true;
		pr_info("Manual hook: applying SELinux rules and setting up cred\n");
	...
	if (!ksu_is_manager_appid_valid()) {
		track_throne(false);
		if (!ksu_is_manager_appid_valid()) {
			pr_warn("KSU bootstrap: setting uid %d as manager\n",
				audit_uid);
			ksu_set_manager_appid(audit_uid);
		}
	}
