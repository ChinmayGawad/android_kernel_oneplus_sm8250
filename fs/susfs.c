...
/* Stubs for wshamroukh KSU-Next-SUSFS module */
void susfs_set_i_state_on_external_dir(void __user **user_info) { pr_info("susfs: set_i_state stub\n"); }
EXPORT_SYMBOL_GPL(susfs_set_i_state_on_external_dir);
void susfs_add_try_umount(void __user **user_info) { pr_info("susfs: add_try_umount stub\n"); }
void susfs_set_hide_sus_mnts_for_all_procs(void __user **user_info) { susfs_set_hide_sus_mnts_for_non_su_procs(user_info); }
void susfs_try_umount(uid_t uid) { pr_info("susfs: try_umount uid=%d stub\n", uid); }
EXPORT_SYMBOL_GPL(susfs_try_umount);
void susfs_reorder_mnt_id(void) { pr_info("susfs: reorder_mnt_id stub\n"); }
